...
interface FlatDeviceData {
  ...
  flatServiceIDs: Uint8Array;
  flatPrices: Uint8Array;
}
...
async getPeerArraySize(contractAddress: string): Promise<number> {
    try {
...
*/
  ...
  async getDevicesByOwner(
    ...
    const [
      ...
      flatServiceIDs,
      flatPrices,
    ] = await contract.getDevicesFlatByOwner(ownerAddress);
  ...
  async getDevicesFlatByOwner(owner: string, contractId: string): Promise<FlatDeviceData> {
    try {
      this.client.setOperator(
        this.operatorId,
        PrivateKey.fromString(this.operatorKey)
  ...
  async getDeviceCountByOwner(owner: string, contractId: string): Promise<number> {
    try {
      this.client.setOperator(
        this.operatorId,
        PrivateKey.fromString(this.operatorKey)
...
async getAllDevices(contractAddress: string,start: number = 0): Promise<Device[]> {
    ...
    for (let i = start; i < peerCount; i++) {
      try {
        const owner: string = await contract.peerList(i);
        ...
        const [
          ...
          flatServiceIDs,
          flatPrices,
        ] = await contract.getDevicesFlatByOwner(owner);
